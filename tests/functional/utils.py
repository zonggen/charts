...
def get_all_charts(charts_path: str, vendor_types: str) -> list:
    ...
    for vt in vendor_types:
        ...
        vendor_names = [name for name in os.listdir(charts_path_vt) if os.path.isdir(f'{charts_path_vt}/{name}')]
        for vn in vendor_names:
            charts_path_vt_vn = f'{charts_path_vt}/{vn}'
            chart_names = [name for name in os.listdir(charts_path_vt_vn) if os.path.isdir(f'{charts_path_vt_vn}/{name}')]
            for cn in chart_names:
                ...
                if 'OWNERS' not in file_names:
                    continue
                chart_versions = [name for name in os.listdir(charts_path_vt_vn_cn) if os.path.isdir(f'{charts_path_vt_vn_cn}/{name}')]
            ...
            for cn in chart_names:
                ...
                for cv in chart_versions:
                    ...
                    file_names = [name for name in os.listdir(
                        ...
                        # TODO: if report exists, don't run
                    if'{cn}-{cv}.tgz' in file_names or 'src' in file_names:
                        ret.append((vt, vn, cn, cv))
    return ret
